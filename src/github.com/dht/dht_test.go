...
func TestDHT1(t *testing.T) {
	...
	//fmt.Println("all finger for node  in list", node3b.printTable())
	node0b.printTable()
	//fmt.Println(node3b.)

	fmt.Println("TEST: " + node0b.lookup("09").nodeId)
	fmt.Println("acc look test: ", node0b.acceleratedLookupUsingFingers("09"))
...
func TestDHT3(t *testing.T) {
	//fmt.Println(between([]byte("2"), []byte("4"), []byte("3")))
	/*fmt.Println("30 ", []byte("30"))
	fmt.Println("030 ", []byte("030"))
	//nodeTest, _ := hex.DecodeString("hej")
	//fmt.Println("hex decode: ", nodeTest)
	fmt.Println("04 ", []byte("04"))
	fmt.Println("4", []byte("4"))*/
	//fmt.Println("distance: ", distance([]byte("2"), []byte("1"), 3))
}
